...
*===============================================================================
*              EQUATIONS for INVEST and DISPATCH
*===============================================================================
...
INV_P2P(p2p)             investments in p2p-technology (fuel cells and h2-storage-tank) [EUR]
C_P2P                    capacity of p2p fc [MW] and h2stor [MWh]
H2_SL                    charging of hydrogen storage [MWh]
GP_FC                    dispatch of fuel cells [MWh per h]
;
...
TOTAL_COSTS =e=   sum((t,exist), G_P1(t,exist) * co_v1(t,exist))
                ...
                + sum(p2p, INV_P2P(p2p) + (char2_p2p(p2p,'co_f_p2p'))* (char2_p2p(p2p,'inst_p2p') + C_P2P(p2p)))
...
energy_balance(t,c)..
               ...
               + sum(fc$map_p2pC(fc,c), GP_FC(t,fc))
...
minimum_p2g(p2g)..
         sum(t,DSM_UP_trans_ind(t,p2g)) =e= sum((c,dev)$ (map_appC(p2g,c) and map_appDev(p2g,dev)),(dem_p2x(c,'p2g','%year%')+ dem_p2x(c,'p2g_ind','%year%'))/ char_dev2(dev,'eta','%year%'))
;
*industry hydrogen production needs to be constant in every hour
         DSM_UP_trans_ind.lo(t,p2g)= sum((c,dev)$ (map_appC(p2g,c) and map_appDev(p2g,dev)),dem_p2x(c,'p2g_ind','%year%')/char_dev2(dev,'eta','%year%'))/8760
;
...
* p2g2p => hydrogen storage and reconversion into electricity by fuel cells

*annuity for hydrogen-storage and fuel cells => p2stor + fc expansion planning
an_p2p(p2p)= sum(fcH2$map_p2pFCH2(p2p,fcH2), char_fcH2(fcH2,'co_inv_p2p','%year%')) *(((1+i)**char2_p2p(p2p,'eco_lf_p2p') * i)/((1+i)**char2_p2p(p2p,'eco_lf_p2p') - 1))
;

*investment for hydrogen storage + fuel cells
investment_p2p(p2p)..
         INV_P2P(p2p) =e= C_P2P(p2p) * an_p2p(p2p)
;

*storage level of hydrogen storage
h2stor_in(t,h2stor)..
         H2_SL(t,h2stor) =e=  H2_SL(t-1,h2stor) + (sum(p2g$map_p2pG(h2stor,p2g), DSM_UP_surplus(t,p2g)))* char_dev2('p2g','eta','%year%')
                                 - sum(fc$map_hstorFC(fc,h2stor), GP_FC(t,fc)/char_fcH2('fc','eta_p2p','%year%'))
;

*maximal storage level
h2stor_in_max(t,h2stor)..
         H2_SL(t,h2stor) =l= C_P2P(h2stor)
;

*maximal fuel cell capacity expansion
fc_max(t,fc)..
         GP_FC(t,fc) =l= C_P2P(fc)
;
