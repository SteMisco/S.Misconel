...
cap_p2x_exist(y,c,dev)   capacity of existing P2X applications [MW] (p2g and p2h)
gen(y,t,tech,c)          hourly generation of power plants and storages [MWh per hour]
...
cap_p2x_new(c,dev)       added + installed P2X capacity per country [MW]
...
cap_p2x_exist(y,c,dev) = sum(app$(map_appC(app,c) and map_appDev(app,dev)), char_app(app,'inst'));
gen(y,t,tech,c)        = sum(p$(map_ptech(p,tech) and map_pc(p,c)),G_P.l(t,p));
...
cap_p2x_new(c,dev)   = sum(app$(map_appC(app,c) and map_appDev(app,dev)),  char_app(app,'inst')) + cap_p2x(c,dev);

pr_el(t,c)  = energy_balance.m(t,c);
pr_el_av(c) = sum(t,pr_el(t,c))/card(t);

f_use(y,tech,c)$eta_new(c,tech) = sum(t,(gen(y,t,tech,c)/eta_new(c,tech)));
...
p2g_use(p2g,c) = sum(t,P2X_UP_trans_ind.l(t,p2g)$map_appC(p2g,c));
pr_el_p2g(p2g,c)$(p2g_use(p2g,c)>0) = sum(t,pr_el(t,c) * P2X_UP_trans_ind.l(t,p2g)$map_appC(p2g,c)) / p2g_use(p2g,c);
flh(p2g,c)$(p2g_use(p2g,c)>0)= p2g_use(p2g,c) /(C_P2X.l(p2g)+char_app(p2g,'inst')$map_appC(p2g,c));

*crossover of barrier algorithm switched off (with solutionstype 2)
...
cap_p2x_new(c,dev)$(sum(app$(map_appC(app,c) and map_appDev(app,dev)),  (char_app(app,'inst') + cap_p2x(c,dev)) < 5.0)) = 0;

execute_unload "%resultfile1%.gdx" ;
execute_unload "%resultfile2%.gdx" cap_new, cap_add, C_ADD.l, eta_new, C_P2X.l, cap_p2x, cap_p2x_new;
